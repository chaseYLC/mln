#include "stdafx.h"
	...
	#define EXTERNC
		...
		void ExceptionHandler::Init(const char* root_path, const char* header, const bool isFullDump)
		{
			...
			// is full dump??
			if (isFullDump)
				g_dump_func = ExceptionHandler::CreateFullDump;
			else
				g_dump_func = ExceptionHandler::CreateMiniDump;

			SetProcessExceptionHandlers();
			SetThreadExceptionHandlers();
		}

		void ExceptionHandler::Init(const char* env_file)
		{
			assert(env_file && "env_file cannot be null. in ExceptionHandler::Init()");

			if (false == InitFromXML(env_file))
			{
				ExceptionHandler::Init(nullptr, nullptr, true);
			}
		}

		bool ExceptionHandler::InitFromXML(const char* env_file)
		{
			...
			ExceptionHandler::Init(strPath.c_str(), strFilename.c_str(), isFullDump);
		...
		void ExceptionHandler::GetExceptionPointers(DWORD dwExceptionCode, EXCEPTION_POINTERS** ppExceptionPointers)
		{
	...
	#pragma warning(pop)
		...
		void ExceptionHandler::CreateMiniDump(EXCEPTION_POINTERS* pExcPtrs)
		{
		...
		void ExceptionHandler::CreateFullDump(EXCEPTION_POINTERS* pExcPtrs)
		{
			HMODULE hDbgHelp = NULL;
			HANDLE hFile = NULL;
			MINIDUMP_EXCEPTION_INFORMATION mei;
		...
		void ExceptionHandler::MyStackWalk(EXCEPTION_POINTERS* pExcPtrs)
		{
			...
			uintptr_t pReserved)
		{
			...
			do
			{
				...
				{
					unsigned long  numOfByteWritten = 0;
					wchar_t buff[BUFFER_COUNT] = { 0, };

					_snwprintf_s(buff, BUFFER_COUNT, _TRUNCATE, L"Exception : %s\r\n", expression);
					WriteFile(hFile, buff, static_cast<DWORD>(wcslen(buff) * sizeof(wchar_t)), &numOfByteWritten, NULL);
					_snwprintf_s(buff, BUFFER_COUNT, _TRUNCATE, L"Function : %s\r\n", function);
					WriteFile(hFile, buff, static_cast<DWORD>(wcslen(buff) * sizeof(wchar_t)), &numOfByteWritten, NULL);
					_snwprintf_s(buff, BUFFER_COUNT, _TRUNCATE, L"File : %s\r\n", file);
					WriteFile(hFile, buff, static_cast<DWORD>(wcslen(buff) * sizeof(wchar_t)), &numOfByteWritten, NULL);
					_snwprintf_s(buff, BUFFER_COUNT, _TRUNCATE, L"Line : %u\r\n", line);
					WriteFile(hFile, buff, static_cast<DWORD>(wcslen(buff) * sizeof(wchar_t)), &numOfByteWritten, NULL);
