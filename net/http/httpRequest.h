...
static const int RECV_TIME_OUT_SEC = 5;
static const int HTTP_REQUEST_RECV_BUFFER_MAX = 65536;
...
namespace http
{
    ...
    inline std::string urlEncode(const std::string& str)
    {
        ...
        {
            const uint8_t cp = (uint8_t)(*i) & 0xFF;
    ...
    {
    public:
        explicit Request(const std::string& url, InternetProtocol protocol = InternetProtocol::V4
        ):
        ...
        Response send(const std::string& method,
                      ...
                      const int recvTimeOutSec = RECV_TIME_OUT_SEC
        )
        {
            std::string body;
            bool first = true;
            ...
            return send(method, body, headers, recvTimeOutSec);
        }
        ...
        )
        {
            ...
            uint8_t tempBuffer[HTTP_REQUEST_RECV_BUFFER_MAX];
            ...
            {
#ifdef _WIN32
                DWORD recvTimeout = recvTimeOutSec * 1000;
                setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (char*)&recvTimeout, sizeof(recvTimeout));
#elif __linux__
                timeval tv;
                tv.tv_sec = recvTimeOutSec;
                tv.tv_usec = 0;
                setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(timeval));
#endif
